...
def main():
    ...
    st.markdown("Aplikasi Web ini memungkinkan User untuk menjalankan serangkaian Autentikasi Wajah berdasarkan arah menghadap Wajah User, yaitu dimulai dari menghadap Depan-Kanan-Kiri agar User dapat masuk ke Sistem. Jika salah satu Autentikasi tidak berhasil maka User tidak akan bisa melanjutkan Autentikasi berikutnya dan User tidak akan berhasil masuk kedalam Sistem. [Coba aplikasi ini dengan Kamera menggunakan model Face Detection Mediapipe](https://4uth-with-mediapipe.streamlit.app/)", unsafe_allow_html=True)

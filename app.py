...
def main():
    ...
    st.markdown("<strong>Karena keterbatasan Dataset yang kami miliki, apabila project ini ingin dikembangkan, kami akan melakukan training data ulang dengan dataset berdasarkan Wajah dari seluruh User (pengguna aplikasi).</strong>", unsafe_allow_html=True)
                    ...
                    if uploaded_file is not None:
                        ...
                        if class_name.strip().lower() == "kiri" and confidence_score > 0.9:
                            ...
                            with st.spinner('Mohon tunggu 5 detik, sedang menuju Sistem...'):
                                ...
                                st.markdown("<h4 style='color: red;'> Perlu diperhatikan bahwa Developer dari proyek ini mempunyai maksud untuk menawarkan Algoritma/Fitur Autentikasi multi faktor yang lebih baik dari autentikasi-autentikasi sebelumnya, sehingga aplikasi memang dibuat sedemikian rupa agar menjadi sederhana. Sangat dimungkinkan jika kedepannya fitur MFA ini dikembangkan tidak hanya berdasarkan pengenalan Wajah, tetapi pengenalan lain juga seperti suara, landmark wajah, dan lainnya. </h4>", unsafe_allow_html=True)
                        else:
